...
for queue_type in df["queue"].unique():
    ...
    plt.legend()
    plt.savefig("%s\\wait_time_%s.png" % (par_fold, queue_type))
    plt.close()
    ...
    plt.legend()
    plt.savefig("%s\\wait_time_with_talk_%s.png" % (par_fold, queue_type))
    plt.close()
    ...
    plt.legend()
    plt.savefig("%s\\talk_time_%s.png" % (par_fold, queue_type))
    plt.close()



mean_time=np.mean(df[:]['wait_time'])
sns.distplot(df[idxs]['wait_time'], kde=False, norm_hist=True, color='r', label="wait_time_%.2f_all__%.2d" % (mean_time,np.sum(idxs)))
plt.legend()
plt.savefig("%s\\wait_time_all.png" % (par_fold))
plt.close()
...
plt.legend()
plt.savefig("%s\\wait_time_all_with_talk.png" % (par_fold))
plt.close()
...
plt.legend()
plt.savefig("%s\\talk_time_all.png" % (par_fold))
plt.close()
